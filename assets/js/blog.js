...
(function () {
  ...
  function renderBody(content = '') {
    const rendered = window.marked ? window.marked.parse(content) : content;
  ...
  async function loadPosts() {
    try {
      ...
      if (!response.ok) throw new Error('Could not load managed posts');
      const data = await response.json();
      if (!Array.isArray(data.posts)) throw new Error('Invalid post data');
      return data.posts;
    } catch (error) {
      console.warn('Using bundled blog fallback.', error);
      return window.TEEJAYE_POSTS || [];
    }
  }
  ...
  function renderPost() {
    ...
    if (!post) {
      ...
      article.innerHTML = `<div class="article-shell article-not-found"><div class="eyebrow">From the Writing Desk</div><h1>That post could not be found.</h1><p>The link may be outdated, or the post may not be published yet.</p><a class="btn teal" href="writing-desk.html">Back to the blog →</a></div>`;
      return;
    }
    ...
    article.innerHTML = `
      <div class="article-shell">
        ...
        <div class="article-content">${renderBody(post.content)}</div>
        <div class="article-end"><p><strong>Thanks for reading.</strong> Explore more notes from the writing desk.</p><div class="button-row"><a class="btn teal" href="writing-desk.html">More posts</a></div></div>
      </div>`;
  }
